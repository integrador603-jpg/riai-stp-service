...
def analyze_step(filepath):
    ...
    # ── orientations for packing (6 rotations) ──────────────────
    dims = sorted([L, W, H])
    orientations = [
        {'label': 'L×W×H', 'x': L, 'y': W, 'z': H},
        {'label': 'W×L×H', 'x': W, 'y': L, 'z': H},
        {'label': 'H×W×L', 'x': H, 'y': W, 'z': L},
        {'label': 'L×H×W', 'x': L, 'y': H, 'z': W},
        {'label': 'W×H×L', 'x': W, 'y': H, 'z': L},
        {'label': 'H×L×W', 'x': H, 'y': L, 'z': W},
    ]
    ...
    return {
        ...
        'note': (
            ...
            if is_simple else
            f'Pieza compleja ({n_faces} caras). '
            'Se recomienda usar el bounding box global para empaquetamiento.'
        ),
    }
